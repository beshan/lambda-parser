...
var Data = class {
  ...
  create(entry) {
    ...
    if (!propName)
      throw new Error("Entry name can't be undefined." /* UndefinedStackEntryName */);
  ...
  getDatasource(entry) {
    ...
    if (!parent)
      return this.source;
    if (parent.rule.isChoice)
      return this.getDatasource(parent);
    if (parent.data === void 0) {
      throw new Error("Entry's parent data is undefined, create(entry.parent) should be called first." /* UndefinedEntryParentData */);
    }
    return parent.data;
  }
};
...
var object = {
  ...
  trim: (obj, what = [void 0], callback) => {
    if (obj instanceof Array) {
      ...
      if (object.isEmpty(result) && what.includes("[]"))
        return;
      return result;
    }
    if (obj instanceof Object) {
      ...
      if (object.isEmpty(obj) && what.includes("{}"))
        return;
      return callback ? callback(obj) : obj;
    }
    if (what.includes(obj))
      return;
    return obj;
  },
  isEmpty: (obj) => {
  ...
  filterProps(obj, namePattern) {
    if (!(namePattern instanceof RegExp))
      namePattern = string.toRegExp(namePattern);
    const result = [];
    for (const key in obj) {
      if (key.match(namePattern))
        result.push(key);
    }
    return result;
  },
...
var Parser = class {
  constructor(grammar, settings) {
    ...
    this.skipPattern = (settings == null ? void 0 : settings.skip) ? new RegExp(`^(${settings == null ? void 0 : settings.skip.source})`) : void 0;
    this.listener = (event) => {
      var _a2, _b;
  ...
  parse(input) {
    ...
    while (this.stack.length) {
      ...
      if (!entry.rule.isPrepared) {
        if (entry.rule.skipPattern)
          entry.rule.skipPattern = new RegExp(`^(${entry.rule.skipPattern.source})`);
        if (entry.rule.isRegExp)
          entry.rule[0] = new RegExp(`^(${entry.rule[0].source})`);
        entry.rule.isPrepared = true;
      }
      entry.startPosition = this.lexer.position;
      if (entry.rule.skipPattern)
        this.lexer.scan(entry.rule.skipPattern);
      if (entry.rule.isAssertion) {
        ...
        const succeeded = !func.fails(() => assertionParser.parse(this.lexer.unscannedInput)) || assertionParser.lexer.scannedInput.length > 0 && !assertionParser.stack.length;
        if (!succeeded && entry.rule.isLookaheadAssertion || succeeded && entry.rule.isNegativeLookaheadAssertion)
          this.failed(entry);
        else
          this.succeeded(entry, void 0);
      } else {
        if (entry.name)
          this.data.create(entry);
        if (entry.rule.isRegExp) {
          const lexeme = this.lexer.scan(entry.rule[0]);
          if (lexeme === void 0)
            this.failed(entry);
          else
            this.succeeded(entry, lexeme);
          if (!this.stack.length && !this.lexer.isEof && entry.rule.skipPattern)
            this.lexer.scan(entry.rule.skipPattern);
          ...
          if (callbackRule instanceof Function) {
            ...
            if (lexeme === void 0)
              this.failed(parent);
            else if (!parent.rule.isClosure)
              this.succeeded(parent, lexeme);
          } else {
            const _rule = __spreadValues({}, callbackRule);
            if (entry.rule.isLast)
              _rule.isLast = entry.rule.isLast;
            if (entry.parent) {
          ...
          for (let i = entry.rule.length - 1; i >= 0; i--)
            this.push(entry.rule[i], entry);
        }
      }
    }
    if (this.skipPattern)
      this.lexer.scan(this.skipPattern);
    if (!this.lexer.isEof)
      this.log("error" /* Error */, "Unable to continue parsing.");
    if (this.loggingEnabled)
      console.log("Parsing done in %d steps:", this.step);
  ...
  push(rule, parent) {
    if (typeof rule !== "string" && !rule.skipPattern) {
      ...
      else if (this.skipPattern)
        rule.skipPattern = this.skipPattern;
    }
    if (parent)
      parent.startPosition = this.lexer.position;
    if (!rule.name && !rule.isCallback && !rule.isRegExp)
      this.setRandomNameForUnnamedRule(rule);
    const entry = {
      rule,
  ...
  succeeded(entry, lexeme, isClosureSuccess) {
    var _a;
    if (this.loggingEnabled)
      console.log("Parsed: %s", lexeme);
    if (entry.name) {
      if (entry.rule.isNonCapture)
        this.data.delete(entry);
      else {
        if (!entry.rule.isClosure || isClosureSuccess) {
          const modification = (_a = this.listener) == null ? void 0 : _a.call(this, {
        ...
        if (lexeme !== void 0) {
          if (isClosureSuccess)
            this.data.getDatasource(entry)[entry.name] = lexeme;
          else
            this.data.set(entry, lexeme);
        }
      }
    }
  ...
  failed(failedEntry) {
    ...
    do {
      if (this.resolveFailureByAlternativeRule(entry) || this.resolveFailureByClosureRule(entry) || this.resolveFailureByOptionalRule(entry))
        return;
    } while (entry = entry.parent);
    const resolved = (_a = this.listener) == null ? void 0 : _a.call(this, {
      type: 1 /* RuleFailed */,
      data: {
      ...
      if (data instanceof Array) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (!object.isEmpty(obj))
            result.push(object.isMap(obj) ? map.toArray(obj) : obj);
        }
        return result;
      }
  ...
  spreadUnnamedProps(obj) {
    for (const key in obj) {
      const prop = obj[key];
      if (prop === void 0 || !key.startsWith(UNNAMED_ENTRY_NAME))
        continue;
      delete obj[key];
      if (prop === null)
        continue;
  ...
  propagateSuccess(entry) {
    ...
    let isClosure = false;
    while (parent) {
      if (parent.rule.isNonCapture)
        this.data.delete(entry);
      else {
        isClosure || (isClosure = parent.rule.isClosure || ((_a = parent.parent) == null ? void 0 : _a.rule.isOrderedChoice) && ((_b = parent.parent) == null ? void 0 : _b.rule.isClosure));
        if (!isClosure && !parent.rule.isOrderedChoice) {
          const data = this.data.get(parent);
          const modification = (_c = this.listener) == null ? void 0 : _c.call(this, {
            type: 0 /* RuleSucceeded */,
            data: {
              data,
              name: parent.name,
          ...
          if (modification !== void 0)
            this.data.set(parent, modification);
        }
      }
      this.cancelAlternativeRules(parent);
      if (!parent.rule.isLast)
        break;
      parent = parent.parent;
    }
  }
...
var ExpressionFactory = class {
  static compile(name, entries, skip, combinator = "." /* Sequence */) {
    ...
    if (combinator === "." /* Sequence */)
      ...
      throw new Error("Unknown combinator. Please use '.' for sequence, or '/' for ordered-choice." /* UnknownCombinator */);
    for (let i = 0; i < entries.length; i++) {
      const entry = entries[i];
      let expr;
      if (combinator === "." /* Sequence */ && entry instanceof Array)
        throw new Error(string.format("Failed to compile '#{0}' rule, entry at index #{1} can't be an array, please use ...[] instead." /* ArrayInSequenceRuleDefinitionEntry */, name, i));
      expr = combinator === "." /* Sequence */ ? this.compileSequenceEntry(entry) : expr = this.compileChoiceEntry(name, entry, skip);
    ...
    if (combinator === "." /* Sequence */) {
      for (let i = 0; i < entries.length; i++) {
        ...
        if (entry instanceof RegExp && last instanceof RegExp)
          merged[merged.length - 1] = new RegExp(
            (merged.length === 1 ? `(${last.source})` : last.source) + ((skip == null ? void 0 : skip.source) ? `(${skip == null ? void 0 : skip.source})` : "") + `(${entry.source})`
          );
        else
          merged.push(entry);
      }
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (!(entry instanceof RegExp))
          ...
          pattern += `${pattern ? "|" : ""}(${entry.source})`;
      }
      if (pattern)
        merged.unshift(new RegExp(pattern));
    }
    return merged;
  }
  static mergeQuantifiers(first, second) {
    ...
    if (quantifiers.includes("*"))
      return "*";
    if (quantifiers.includes("+") && quantifiers.includes("?"))
      return "*";
    return first;
  }
  static applyModifiersToRegExpRule(rule, { quantifier, assertion }) {
    if (quantifier)
      rule[0] = new RegExp(`(${rule[0].source})${quantifier}`);
    if (assertion)
      rule[0] = new RegExp(`(${assertion}(${rule[0].source}))`);
  }
};
...
var RuleFactory = class {
  ...
  static clone(entries, {
    ...
    if (quantifier)
      factory.quantify(quantifier);
    if (combinator)
      factory.combine(combinator);
    if (capture === false)
      factory.capture(false);
    if (skip !== void 0)
      factory.skip(skip);
    if (debug)
      factory.debug();
  ...
  quantify(quantifier) {
    if (!["?" /* Optional */, "*" /* ZeroOrMore */, "+" /* OneOrMany */].includes(quantifier))
      throw new Error("Unknown quantifier. Please use '?' for optional, or '*' for zero-or-more, or '+' for one-or-more." /* UnknownQuantifier */);
    this.quantifier = quantifier;
    return this;
  }
  assert(assertion) {
    if (!["?=" /* Lookahead */, "?!" /* NegativeLookahead */].includes(assertion))
      throw new Error("Unknown assertion. Please use '?=' for lookahead, or '?!' for negative-lookahead." /* UnknownAssertion */);
  ...
  as(name) {
    ...
    if (this.quantifier)
      rule.quantifier = this.quantifier;
    if (this.assertion)
      rule.assertion = this.assertion;
    if (this.combinator)
      rule.combinator = this.combinator;
    if (this.debuggerEnabled)
      rule.debuggerEnabled = true;
    if (this.skipPattern && !rule.isRegExp) {
      rule.skipPattern = this.skipPattern;
    }
    if (this.isNonCapture)
      rule.isNonCapture = true;
    this.optimize(rule);
    ...
    if (rule.isRegExp || rule.length === 1 && firstSubRule.isRegExp) {
      ...
      if (firstSubRule.isRegExp)
        rule.isSequence = true;
      return rule;
    }
    if (rule.quantifier) {
  ...
  optimize(rule) {
    var _a, _b;
    const firstSubRule = rule[0];
    if (rule.length > 1 || rule.isRegExp || rule[0].isCallback || firstSubRule.name && rule.name || // sub.combinator !== this.combinator ||
    rule.isChoice || firstSubRule.isNonCapture === false || rule.skipPattern && firstSubRule.skipPattern && rule.skipPattern !== firstSubRule.skipPattern)
      return;
    ...
    Object.assign(rule, __spreadProps(__spreadValues({}, firstSubRule), {
      name,
      quantifier,
      assertion,
      combinator,
